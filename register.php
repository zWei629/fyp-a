...
<head>
    <title>O.W Skincare : Register</title>
...
<body>
    ...
    if (isset($_POST['ok'])) {
        ...
        $repwd = $_POST['repwd'];
        if ($pwd == $repwd) {
            $qry1 = mysqli_query($con, "SELECT * FROM user WHERE Email = '$email'") or die(mysqli_error($con));
            $qry2 = mysqli_num_rows($qry1);
            if ($qry2 == 0) {
                $qry3 = mysqli_query($con, "INSERT INTO user (Name, Email, Dob, Gender, Address, Phone, Password) VALUES ('$name', '$email', '$dob', '$gnd', '$addr', '$phno','$pwd')") or die(mysqli_error($con));
                echo '
                <script>
